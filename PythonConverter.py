...
class ConversionHandler(FileSystemEventHandler):
    ...
    def convert_to_sdr(self, file_path):
        try:
            ...
            subprocess.run(["hdrfix --exposure=-1", file_path, output_file], check=True, shell=True)
...
def main():
    ...
    Checkbutton(root, text="Keep Original Files", variable=delete_original_var, state=DISABLED).grid(row=2, column=0, padx=5, pady=5, sticky="w")
